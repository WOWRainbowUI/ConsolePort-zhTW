...
function BindingsPanel:SaveBindings()
	...
	if (set == CHARACTER_BINDINGS) then
		CPAPI.Log('已儲存 %s 的搖桿按鈕綁定。', CPAPI.GetPlayerName(true))
	else
		CPAPI.Log('已儲存搖桿按鈕綁定。')
	end
			...
			end;
			{
				...
				Import = {
					...
					_OnClick = function()
						...
						self.Control.Footer:SetText(state and '設定檔' or CATEGORIES)
				...
				Cancel = {
					...
					TooltipText = '放棄所有變更，回復成先前的設定。';
					_OnClick = function(self)
						CPAPI.Popup('ConsolePort_Previous_Keybindings', {
							text = CONFIRM_RESET_TO_PREVIOUS_KEYBINDINGS or '是否要重置所有按鈕綁定，恢復成先前的設定?';
							button1 = OKAY;
							button2 = CANCEL;
							timeout = 0;
							whileDead = 1;
	...
	mapper = self:CreateScrollableColumn('Mapper', {
		...
		};
		{
			Catch = {
				...
				PopupText = ('\n|cFFFFFF00%s|r\n\n%s\n\n'):format('設定按鈕綁定', BIND_KEY_TO_COMMAND:gsub(' %->', ':\n'));
			};
			Child = {
				_Width = LEFT_PANEL_WIDTH;
				{
