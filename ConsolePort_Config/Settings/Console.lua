...
function DeviceSelect:Construct()
	...
	Widgets.Select(self, 'DeviceID', nil, db.Data.Select(1, 1):SetRawOptions(options), '裝置資訊')
...
function DeviceSelect:Update()
	...
	if device then
		...
		self.tooltipText = ('名稱: %s\n廠商 ID: |cFF00FFFF%s|r / |cFF00FF00%s|r\n產品 ID: |cFF00FFFF%s|r / |cFF00FF00%s|r'):format(
			device.name,
		...
		self.tooltipText = self.tooltipText .. '\n\n'
			.. YELLOW_FONT_COLOR:WrapTextInColorCode('點這裡重置你的裝置設定檔。');
	else
		self:SetText('從清單中選擇裝置然後繼續。')
	end
