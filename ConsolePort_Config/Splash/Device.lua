...
function Device:OnClick()
	...
	CPAPI.Popup('ConsolePort_Apply_Preset', {
		text = L('是否要載入設定給 %s 使用?\n\n這會設定輔助鍵、滑鼠模擬按鈕和之前儲存的裝置設定 (如果有的話)。', device.Name);
		button1 = OKAY;
		button2 = CANCEL;
		timeout = 0;
		whileDead = 1;
		...
		OnHide = function()
			CPAPI.Popup('ConsolePort_Reset_Keybindings', {
				text = ('%s\n\n%s'):format(CONFIRM_RESET_KEYBINDINGS, '只會影響搖桿按鈕綁定');
				button1 = OKAY;
				button2 = CANCEL;
				timeout = 0;
				whileDead = 1;
				...
				OnHide = function()
					if device:ConfigHasBluetoothHandling() then
						CPAPI.Popup('ConsolePort_Apply_Config', {
							text = L('你的 %s 搖桿可以選擇使用藍芽或無線連接。\n請問要使用哪一種?', device.Name);
							button1 = '無線';
							button2 = CANCEL;
							button3 = '藍芽';
							timeout = 0;
