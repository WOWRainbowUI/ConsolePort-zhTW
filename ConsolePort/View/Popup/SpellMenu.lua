...
function SpellMenu:SetCommands()
	...
	self:AddCommand('放到快捷列上', 'MapActionBar')
...
function SpellMenu:MapActionBar(keyChord)
	...
	if keyChordSlug then
		...
		description = L('選擇一個欄位綁定 %s 和放置這個法術。', keyChordSlug);
		self:DisplayBindingsForPending(keyChordSlug)
	else
		description = L'選擇要放置這個法術的欄位。';
		keyChord = nil;
	end
	...
	if leftClick then
		handle:AddHint(leftClick, '放入欄位')
	end
	if rightClick then
		handle:AddHint(rightClick, keyChord and L'取消並清除游標' or L'清空欄位或按鈕綁定')
	end
	if specialClick and not keyChord then
		handle:AddHint(specialClick, L'設定按鈕綁定')
	end
	if cancelClick then
