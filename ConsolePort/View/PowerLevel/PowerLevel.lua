...
PowerLevel.Levels = CPAPI.Proxy({
	{fill = 1, color = RED_FONT_COLOR,    atlas = 'ui-frame-bar-fill-red',    name = '吃緊', animation = 'Critical'};
	{fill = 1, color = ORANGE_FONT_COLOR, atlas = 'ui-frame-bar-fill-yellow', name = '低'};
	{fill = 2, color = YELLOW_FONT_COLOR, atlas = 'ui-frame-bar-fill-yellow', name = '中'};
	{fill = 3, color = GREEN_FONT_COLOR,  atlas = 'ui-frame-bar-fill-green',  name = '高'};
	{fill = 3, color = BLUE_FONT_COLOR,   atlas = 'ui-frame-bar-fill-blue',   name = '充電中', animation = 'Charging'};
	{fill = 0, color = WHITE_FONT_COLOR,  atlas = 'ui-frame-bar-fill-white',  name = '未連接'};
}, function() return
...
function PowerLevel:OnEnter()
	...
	GameTooltip:AddLine('Shift + 左鍵拖曳移動')
	GameTooltip:Show()
end
