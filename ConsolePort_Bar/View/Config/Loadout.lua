...
local COPY = CALENDAR_COPY_EVENT or '複製';
...
local function ValidatePresets(presets)
	...
	for id, preset in pairs(presets) do
		CPAPI.Log('正在檢查預先設定 %s...', BLUE_FONT_COLOR:WrapTextInColorCode(id))
		if not env.IsV1Layout(preset) and not env.IsV2Layout(preset) then
			CPAPI.Log('預設設定%s不是有效的版面配置。', ORANGE_FONT_COLOR:WrapTextInColorCode(id))
			return false;
		end
	end
	CPAPI.Log('所有預先設定都有效。')
...
---------------------------------------------------------------
	...
	Export = {
		tooltipTitle = L'匯出';
		tooltipText  = L'將此預先設定匯出為可以分享給其他人的字串。';
...
Loadout.LayoutControls = {
	...
	{
		tooltipTitle = SAVE;
		tooltipText  = L'將目前所載入的儲存到預先設定的清單中。';
...
Loadout.PresetControls = {
	...
	{
		tooltipTitle = L'全部匯出';
		tooltipText  = L'將所有的自訂預先設定匯出成可以分享給其他人的字串。';
		icon         = CPAPI.GetAsset([[Textures\Frame\Export]]);
		iconSize     = 18;
		onClickHandler = function(self)
			...
			local popupName = 'ConsolePort_Loadout_Export';
			local popupData = parent.Popups[popupName];
			CPAPI.Popup(popupName, popupData, L'預設設定', nil, {
				owner  = parent;
...
Loadout.Popups = {
	...
	ConsolePort_Loadout_Confirm_Save = {
		...
		button2   = CANCEL;
		enterClicksFirstButton = true;
		text = L('儲存來自%s的預先設定:', YELLOW_FONT_COLOR:WrapTextInColorCode('%s'));
		OnShow = function(popup, data)
	...
	ConsolePort_Loadout_Export = {
		button1    = OKAY;
		hasEditBox = 1;
		text = L('將%s匯出成字串:', YELLOW_FONT_COLOR:WrapTextInColorCode('%s'));
		enterClicksFirstButton = true;
		OnShow = function(popup, data)
...
function Loadout:GetPresetSaveFrame()
	if not self.presetSaveFrame then
		...
		frame.options = CreateCheckBox(
			L'匯出目前選項',
			L('在預先設定資料中包含來自 %s 標籤頁面目前的選項。', BLUE_FONT_COLOR:WrapTextInColorCode(OPTIONS))
		);

		frame.pager = CreateCheckBox(
			L'匯出動作頁面邏輯',
			L'在預先設定資料中包含目前的動作頁面邏輯。'
		);
...
function Loadout:DrawPresets(layoutIndex)
	self.presetHeader = self:CreateHeader('預設設定')
	...
	for _, data in ipairs(GetPresets()) do
		...
		if not data.readonly then
			...
			deleteButton:SetTarget(PATH('Presets', data.id), widget, L'預設設定')
...
function Loadout:DrawConfiguration(layoutIndex)
	self.layoutHeader = self:CreateHeader('載入')
